...
# 1. Back up original files if backup doesn't exist already
docx_files = glob.glob(os.path.join(appendix_dir, "*.docx"))
for f in docx_files:
    fname = os.path.basename(f)
    backup_path = os.path.join(backup_dir, fname)
    if not os.path.exists(backup_path):
        print(f"Backing up: {fname} to backup_original/")
        shutil.copy2(f, backup_path)

# 2. Define the replacement data for each department
# Keys are parts of the file name
DEPT_DATA = {
    "KHPT": {
...
# Find all docx files in Phụ lục
docx_files = glob.glob(os.path.join(appendix_dir, "*.docx"))
print(f"Found {len(docx_files)} files to update.")
...
for fpath in docx_files:
    ...
    # Identify department data based on filename matching
    dept_key = None
    ...
    for t_idx, table in enumerate(doc.tables):
        ...
        if len(header) >= 4 and "Mã yêu cầu" in header[0] and "Tiêu chí chấp nhận" in header[3]:
            ...
            print(f"  Updated Requirements table ({len(table.rows)} rows)")
        ...
        # B. Target Traceability Table
            ...
            for trace in data["traceability"]:
                ...
                for col_idx, val in enumerate(trace):
                    row_cells[col_idx].text = val
            print(f"  Updated Traceability table ({len(table.rows)} rows)")
        ...
        # C. Target UAT Table
            ...
            for uat in data["uat"]:
                ...
                for col_idx, val in enumerate(uat):
                    row_cells[col_idx].text = val
            print(f"  Updated UAT table ({len(table.rows)} rows)")
            
    doc.save(fpath)
    print(f"Success! Updated and saved: {basename}")

print("\nAll docx files successfully updated!")

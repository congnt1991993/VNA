...
try {
    ...
    $sheet2.Cells.Item(130, 3).Value = "Format Test Phân Hệ"
    
    Write-Output "Formatting row 130..."
    for ($c = 1; $c -le 7; $c++) {
        $cell = $sheet2.Cells.Item(130, $c)
        $cell.Interior.Color = 65535 # Yellow
        $cell.Borders.LineStyle = 1 # xlContinuous
        $cell.Borders.Weight = 2 # xlThin
    }
    
    Write-Output "Value immediately after write and format: '$($sheet2.Cells.Item(130, 3).Value)'"
    Write-Output "Text immediately after write and format: '$($sheet2.Cells.Item(130, 3).Text)'"
